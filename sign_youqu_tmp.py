...
def chat_in_group(c, group_id, group_name, user_id, content):
    url = "http://iyouqu.com.cn:8080/app/group/service.do"
...
def get_group_info(c, group_id, user_id):
    ...
    """
    url = "http://iyouqu.com.cn:8080/app/group/service.do"
    params = {'msgId': 'APP079', 'id': group_id, 'limit': 0, 'ids': [user_id]}
    data = json.dumps(params, sort_keys=False, separators=(',', ':'),
                      ensure_ascii=False)
    # print data
    body = {"text": data}
...
def chat_all_user_in_group(c, userlist, group_id, group_name, interval):
    ...
    try:
        ...
        for user in userlist:
            ...
            chat_resp, chat_body = chat_in_group(c, group_id, group_name, userid, LAST_CHAT_CONTENT)
            LOG.info("User %(name)s :chat in youqu, resp: %(resp)s, "
                     "body: %(body)s", {'name': username,
                                        'resp': chat_resp,
                                        'body': chat_body})
...
class YOUQU:
    ...
    def __init__(self):
        ...
        self.userlist = body["resultMap"]["objList"]
        self.group_name = body["resultMap"]["name"]
    ...
    def chat_all(self, args=None):
        ...
        try:
            ...
            LOG.error(e.message)
            ulist = self.userlist
        chat_all_user_in_group(self.c, ulist, self.group_id,self.group_name,
                               self.chat_interval)
